...
class Player{
	...
	private void handle_Ambassador(Player p1, Player p2){
		int randomVal = 0; 
		if (this.Player_Hand.size() != 0)
			randomVal = RandNumbers.getRand(Player_Hand.size());
		int found = 0, i = 0;
		boolean added= false; 
		...
		while(i != Deck.getActionDeck().size()){
			if (Deck.getActionDeck().get(i).size() != 0){
				if (Deck.getActionDeck().get(i).get(0).getName() == RandCard.getName()){
					for (int j = 0; j < PlayDominion.Players.size(); j++){
						Player cur = PlayDominion.Players.get(j);
						if (this.getName() != cur.getName() && Deck.getActionDeck().get(i).size() != 0){
							cur.Player_Deck.add(Deck.getActionDeck().get(i).remove(0));
							System.out.println("Added: " + cur.Player_Deck.toString());
							added = true; 
						}
					}
				}
		...
		while(i != Deck.getVCDeck().size()){
			if (Deck.getVCDeck().get(i).size() != 0){
				if (Deck.getVCDeck().get(i).get(0).getName() == RandCard.getName()){
					for (int j = 0; j < PlayDominion.Players.size(); j++){
						Player cur = PlayDominion.Players.get(j);
						if (this.getName() != cur.getName() && Deck.getVCDeck().get(i).size() != 0){
							cur.Player_Deck.add(Deck.getVCDeck().get(i).remove(0));
							System.out.println("Added: " + cur.Player_Deck.toString());
							added = true; 
						}
					}
		...
		System.out.println(Deck.getMoneyDeck().toString());
		i = 0;
		String test = "Copper";
			...
			if (test == test2 && Deck.getMoneyDeck().get(i).size() != 0){
				for (int j = 0; j < PlayDominion.Players.size(); j++){
					...
					if (this.getName() != cur.getName()){
						cur.Player_Deck.add(Deck.getMoneyDeck().get(i).remove(0));
						System.out.println("Added: " + cur.Player_Deck.toString());
						added = true; 
					}
				}
