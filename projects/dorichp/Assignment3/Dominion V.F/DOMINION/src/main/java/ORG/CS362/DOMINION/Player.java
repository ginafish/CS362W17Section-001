...
//		   state->numnumBuys = 1;
	 ...
	 //ASSUMES THAT DECK HAS THE CARD 
	   final boolean gain(Card card) {
		   
		   if(gameState.gameBoard.get(card) == null || gameState.gameBoard.get(card) <= 0){
			   	return false;
		   }
		   
		   
		//     System.out.println("GAIN FUNCTION &&&&&&&&& ===== the amount for this card" + gameState.gameBoard.get(card));
		   
		   gameState.gameBoard.put(card, gameState.gameBoard.get(card)-1);		//Adds card back into gameBoard with a decremented count
		   discard.add(card);
	   ...
	   public void playKingdomCard() {
		   ...
		   while (numActions > 0) {
		         ...
		         if (actionCards.size() == 0){
		        	 System.out.println("No action cards in deck");
		        	 return;
		         }
   	...
   	//		System.out.println("BUYCARD RANDOM TEST: " + random);
	   //     System.out.println("BUYCARD FUNCTION: " +gameState.cards.size());
	     //   System.out.println("Size: " +gameState.gameBoard.size());
   			 
   			 Card pulled = gameState.cards.get(random);				//Gets the random card object (reference) from the total card list
   		//	 gameState.cards.remove(random);
   			// gameState.gameBoard.get(pulled);
   			...
   			if (numBuys < 1){		//If the player cannot buy anything, then that is it
   			    ...
   			    gain(Card.getCard(gameState.cards, Card.CardName.Copper));			//If the player can't buy the card he will default to picking up a copper
   				return;	
   			}
   			else{
