...
public class OtherFuncTest {
	...
	@Test
	public void test20() {
		System.out.println('\n' + "TEST TO IMPROVE MUTATIONS KILLED" + '\n');
		List<Card> cards = new ArrayList<Card>(Card.createCards());
		GameState state = new GameState(cards);
		Player player = new Player(state, "player-1");
		state.addPlayer(player);
		player.coins = 50;
		player.numBuys = 5; //not really feasible but hopefully will help find mutations
		player.buyCard();
	...
	public void test21() {
		...
		List<Card> cards = new ArrayList<Card>(Card.createCards());
		for(int i = 0; i < cards.size(); i++){
			if(cards.get(i).getCardName() != Card.CardName.greatHall && cards.get(i).getType() != Card.Type.VICTORY){
				//then it is an action or treasure card and shouldn't have any victory value
				assertEquals(cards.get(i).score(), 0);
			
			}
			if(cards.get(i).getType() != Card.Type.TREASURE){
				//then it is an action or treasure card and shouldn't have any victory value
				assertEquals(cards.get(i).getTreasureValue(), 0);
			
			}
			if(cards.get(i).getCardName() == Card.CardName.Cooper){
				assertEquals(0, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Silver){
				assertEquals(3, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Gold){
				assertEquals(6, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Province){
				assertEquals(8, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Duchy){
				assertEquals(5, cards.get(i).getCost());
				assertEquals(3, cards.get(i).score());
			}
			if(cards.get(i).getCardName() == Card.CardName.Estate){
				assertEquals(2, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Curse){
				assertEquals(0, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Adventurer){
				assertEquals(6, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Ambassador){
				assertEquals(3, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Baron){
				assertEquals(4, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Smithy){
				assertEquals(4, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.councilRoom){
				assertEquals(5, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Feast){
				assertEquals(4, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Gardens){
				assertEquals(4, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.greatHall){
				assertEquals(3, cards.get(i).getCost());
			}
			
			if(cards.get(i).getCardName() == Card.CardName.Mine){
				assertEquals(5, cards.get(i).getCost());
			}
			if(cards.get(i).getCardName() == Card.CardName.Remodel){
				assertEquals(4, cards.get(i).getCost());
			}
		}
	...
	public void test22() {
		...
		player.numBuys = 5; //not really feasible but hopefully will help find mutations
		player.buyCard();
		boolean test = player.gain(cards.get(0));
