...
//		   state->numnumBuys = 1;
		      ...
		      //scoreFromPoints = 0;
		      numBuys = 1;
		      //Shuffle your starting 10 cards (7 Coppers & 3 Estates) and place them face-down as your Deck.  the top
	 ...
	 //card goes in discard,
	   final boolean gain(Card card) {
		   ...
		   int currentNum = gameState.gameBoard.get(card);
		   if(currentNum < 1){
			   return false;
		   }
		   gameState.gameBoard.put(card, currentNum-1);
		     discard.add(card);
	   ...
	   public void buyCard() {
		    System.out.println(" --- --------------------------- --- ");
 			...
 			List<Card> canBuy = new ArrayList<Card>(); // list that will hold all cards that are available to buy
 			while(this.numBuys > 0){ // making sure user can only buy numBuys times.
 				if(this.coins > 0){  //making sure user can't go over the number of coins available
 					while(canBuy.size()<1){ // for the number of cards on the board
 					Random random = new Random();
 					List<Card> keys = new ArrayList<Card>(gameState.gameBoard.keySet());
 					Card randomCard = keys.get(random.nextInt(keys.size()));
 					//int value = gameState.gameBoard.get(randomCard);
 						//Card currCard = gameState.cards.get(numCards); // 
 	 					if(randomCard.cost(randomCard)<=this.coins){
 	 						canBuy.add(randomCard);
 	 					}
 					} 
 					//Random rand = new Random();
 	 				//int  n = rand.nextInt(canBuy.size());//0 until size-1
 	 				Card chosen  = canBuy.get(0);
 	 				gain(chosen);
